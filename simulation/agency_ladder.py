...
from directed_info import directed_information_per_step, mutual_information
...
def _compute_factors(
    ...
    scaffold_steps = n // 4
    x, c, e, u_omega = _simulate(
        n=n, policy=policy, rng=rng,
        scaffold_until=scaffold_steps,
        scaffold_strength=0.9,
    )
    rng_scr = np.random.default_rng(seed + 31)
    x_scrambled = _simulate_scrambled(n=n, c_recorded=c, e_recorded=e, rng=rng_scr)
    ...
    di_e = directed_information_per_step(u_w, q_w.astype(float))
    di_x = directed_information_per_step(e_w, q_w.astype(float))
